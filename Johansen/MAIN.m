...
inj_rate  = 3 * mega * 1e3 / year / co2_rho;
min_rate  = 1 * mega * 1e3 / year / co2_rho;
total_inj = 30; 


well_rate = 6*rand(1,19) * mega * 1e3 / year / co2_rho;
total_rate = co2_rho*sum(well_rate*year/2) / mega/1e3;

disp('-----')
disp([total_rate, total_rate-total_inj])

if (total_inj-total_rate) > 0
    well_rate(20) = (total_inj - total_rate) * mega * 1e3 / year/2 / co2_rho;
else
    well_rate(20) = (total_rate - total_inj) * mega * 1e3 / year/2 / co2_rho;
end

new_total = co2_rho*sum(well_rate*year/2) / mega/1e3;
disp([total_inj, new_total])
