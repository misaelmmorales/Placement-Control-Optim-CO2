...
Main_Directory = '/scratch/sft/bailian_chen/NRAP/SyntheticCase/Heterogeneous_1a_higherK'
...
nColumn_data = [17]                           # The column in which the data measurement is located 
...
if ROMs_validation==1:
    ...
    plt.xlabel('ROM Prediction (Kt)',fontsize=16,fontweight="bold")
...
s.savetxt('sample_for_synthetic_data.txt')
...
#    #plt.savefig("UR_ab.png",bbox_inches='tight')
#    plt.show()
...
#    plt.plot(bin_edges_post[1:],cdf1_post,'b',label="Posterior_M1")
#    plt.plot(bin_edges_post_M2[1:],cdf1_post_M2,'black',label="Posterior_M2")
#    plt.xlabel('Cumulative CO$_2$ Leak (Kt)',fontsize=16,fontweight="bold")
#    plt.ylabel('CDF',fontsize=16,fontweight="bold")
#    plt.rc('xtick',labelsize=14)
#    plt.rc('ytick',labelsize=14)
#    plt.legend(loc='lower right',fontsize=14)
#    plt.savefig("CDF_50.png",bbox_inches='tight')    
#    plt.show()
#
#    #plot histogram
#    plt.figure()
...
#    plt.show()
    ...
    Simulated_M1 = []
    for lines in open('Simulated_data_50_M1.dat'):
        lines = lines.rstrip('\n')
        lines = float(lines)
        Simulated_M1.append(lines)
        
    Simulated_M2 = []
    for lines in open('Simulated_data_50_M2.dat'):
        lines = lines.rstrip('\n')
        lines = float(lines)
        Simulated_M2.append(lines)   
    ...
    #for i in range(0,nDataRealization):
    data_plot_1=np.insert(synthetic_data[49][nTimeSeries*0:nTimeSeries*(0+1)],0,data_train_read_raw0[0][0])
    data_plot_2=np.insert(synthetic_data[49][nTimeSeries*1:nTimeSeries*(1+1)],0,data_train_read_raw0[0][1])
    ax.scatter(np.array(time_point)/360,data_plot_1,facecolors='none', edgecolors='r',label="M1_ROM Prediction")   
    ax.scatter(np.array(time_point)/360,data_plot_2,facecolors='none', edgecolors='b',label="M2_ROM Prediction")
    ax.plot(np.array(time_point)/360,Simulated_M1,'r',label="M1_Simulation")   
    ax.plot(np.array(time_point)/360,Simulated_M2,'b',label="M2_Simulation")
    ax.set_xlabel('Time (Years)',fontsize=16,fontweight="bold")
    ax.set_ylabel('Monitoring Pressure (MPa)',fontsize=16,fontweight="bold")
    #plt.ylabel('CO$_2$ Saturation',fontsize=16,fontweight="bold")
    plt.rc('xtick',labelsize=14)
    plt.rc('ytick',labelsize=14)
    plt.xlim([0,Total_time/360])
    plt.ylim([10.200,10.235])
    figname = 'data_realizations_50'
...
if Sensitivity_flag==1: 
    CorrCoeff_All=np.zeros(4)
    for i in range(0,4):
        CorrCoeff_All[i]=np.corrcoef(LHSamples_scaled.T[i],synthetic_data.max(1))[0][1]
    # plot
    plt.figure()
    objects = ('K_L1', 'K_L2',  'K_L3', 'K_Reservoir')
    y_pos = np.arange(len(objects))
    performance = CorrCoeff_All        
    ...
    plt.ylim([-0.24,1])
    plt.xlabel('Uncertain Parameters',fontsize=16,fontweight="bold")
    plt.ylabel('Correlation Coefficient',fontsize=16,fontweight="bold")
    figname = 'Sensitivity_M1'
    plt.savefig(figname,bbox_inches='tight') 
    plt.show()
...
#    #plt.savefig("boxplot_samples_remained.png",bbox_inches='tight')
#    #plt.show()
#    
#    print ''
