...
Main_Directory = '/scratch/sft/bailian_chen/NRAP/Heterogeneous_1a_higherK'
...
eps = 0.002  # for pressure                  # History match error tolerance
#eps = 0.002 # for tempeture
#eps =  0.05 # for liquid CO2 saturation

nColumn_data = 7                            # The column in which the data measurement is located 
nData_read =  60                             # The actural number of data read from simulation
nData      =  60                             # The number of measurement data points
...
ROMs_validation = 1                          # ROMs cross-validation 
...
data_train_read_raw = np.zeros((nTrain,nData_read+1))
data_train_read     = np.zeros((nData_read,nTrain))
data_train = np.zeros((nData,nTrain))
time_set = []
time_point = np.zeros(nData+1)
for itrain in range(0, nTrain):
    ...
    for lines in open(train_data_filename):
        ...
        if count>4:
            ...
            data_train_read_raw[itrain][count-5] = xi
time_point_read = time_set[:nData_read+1]
data_train_read = data_train_read_raw.T[1:nData_read+1]
if nData_read==nData:
...
post_p90mp10_mean, post_p90mp10_time, post_p90mp10_iData, post_mean, post_mean_iData, nSamples,mc_obj_post = Uncertainty_calc(mc_data,synthetic_data,mc_obj,err_option,eps,time_sensitivity)
...
if post_processing==1:
    ...
    plt.figure()
    num_bins1=50
    plt.hist(mc_obj/1e6,bins=num_bins1,color='blue',label='Prior',alpha=0.9)
    plt.hist(mc_obj_post[0]/1e6,bins=num_bins1,color='orange',label='Posterior_R1',alpha=0.9)
    plt.hist(mc_obj_post[99]/1e6,bins=num_bins1,color='red',label='Posterior_R100',alpha=0.9)
    plt.xlabel('Cumulative CO$_2$ Leak (Kt)',fontsize=16,fontweight="bold")   
    ...
    plt.figure()
    for i in range(0,nDataRealization):
        data_plot=np.insert(synthetic_data[i],0,data_train_read_raw[0][0])
        plt.plot(np.array(time_point)/360,data_plot)
    plt.xlabel('Time (Years)',fontsize=16,fontweight="bold")
    plt.ylabel('Temperature ($^\circ$C)',fontsize=16,fontweight="bold")
